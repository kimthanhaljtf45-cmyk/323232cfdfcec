...
export default function MarketplaceScreen() {
  ...
  const { data: locations, isLoading: locationsLoading, refetch: refetchLocations } = useQuery<Location[]>({
    ...
    queryFn: async () => {
      ...
      if (selectedDistrict !== 'Всі райони') {
        return result.filter((loc: Location) => loc.district === selectedDistrict || loc.city === selectedDistrict);
      }
      return result;
    },
  });
  ...
  const { data: coaches, isLoading: coachesLoading } = useQuery<Coach[]>({
    ...
    queryFn: async () => {
      const result = await api.get('/users/coaches');
      return result.filter((c: any) => c.isActive !== false);
    },
  });
